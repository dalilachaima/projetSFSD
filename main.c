...
void display_files() {
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if(file[i].meta.organisation_globale == 1 ){ char orgGlo = "contiguë";}else{char orgGlo = "chaînée";}
        if(file[i].meta.organisation_interne == 1){char orgInt ="trié"}else{char orgInt ="non trié"}
...
// Allocation des blocs
    ...
    int adresse_bloc_precedent = -1; // Variable pour chaîner les blocs
    ...
    for (int i = 0; i < meta.taille_blocs; i++) {
        ...
        // Écriture du bloc dans le fichier
        fwrite(&buffer, sizeof(BLOC), 1, fichier);

        // Mise à jour du bloc précédent pour chaîner les blocs si nécessaire
        if (meta.mode_globale == 2) {
    ...
    printf("Fichier crée avec succes !");
   fclose(fichier);
}
...
void renommer_fichier() {
    ...
    for (int i = 0; i < nbrMaxFichiers; i++) {
        if (strcmp(files[i].meta.nom_fichier, ancien_nom) == 0) {
            ...
            // Vérifie si le nouveau nom est déjà pris
            int nom_deja_pris = 0;
            ...
            // Mise à jour du nom du fichier dans les métadonnées
...
void suppression_logique(int id) {
    ...
    if (/*rechercher_enregistrement selon les modes d'organisation */ ) { //position et bloc trouvés
        Fichier file = files[0]; // Supposons qu'on travaille sur le premier fichier, ajuster si nécessaire
        BLOC bloc;
        ...
        // Vérifier si l'enregistrement existe et marquer comme supprimé
        if (bloc.enreg[posEnreg].ID == id) {
            bloc.enreg[posEnreg].supprime = 1; // Marquer l'enregistrement comme supprimé
            fseek(fichier, posBloc * sizeof(BLOC), SEEK_SET);
...
void suppression_physique(int id) {
    ...
    // Supprimer l'enregistrement en décalant les autres enregistrements
    for (int i = posEnreg; i < bloc.nbrEnreg - 1; i++) {
        bloc.enreg[i] = bloc.enreg[i + 1];
    }
    bloc.nbrEnreg--; // Réduire le nombre d'enregistrements dans le bloc

    // Réécrire le bloc modifié
    fseek(fichier, posBloc * sizeof(BLOC), SEEK_SET);
    fwrite(&bloc, sizeof(BLOC), 1, fichier);

    // Si le bloc devient vide, supprimer le bloc et réorganiser les liens si le fichier est chaîné
    if (bloc.nbrEnreg == 0) {
        int bloc_precedent = -1, bloc_courant = file.meta.adresse_premier_bloc;

        // Trouver le bloc précédent pour mettre à jour le lien
        ...
        // Réorganiser les liens dans le cas de fichiers chaînés
        if (bloc_precedent != -1) {
            ...
            // Si c'est le premier bloc, mettre à jour l'adresse du premier bloc dans les métadonnées
            file.meta.adresse_premier_bloc = bloc.suivant;
        }

        // Réinitialiser le bloc supprimé
        bloc.is_occupied = 0;
        bloc.suivant = -1;
        fseek(fichier, posBloc * sizeof(BLOC), SEEK_SET);
        fwrite(&bloc, sizeof(BLOC), 1, fichier);
...
void insertion_chainee_triee(ENREGISTREMENT enreg) {
    ...
    while (currBloc != -1) {
        ...
        int i = 0;
        while (i < bloc.nbrEnreg && bloc.enreg[i].Clé < enreg.Clé) {
            i++;
        }
        ...
        if (bloc.nbrEnreg < FB) {
            ...
            bloc.enreg[i] = enreg; // Insérer le nouvel enregistrement
            bloc.nbrEnreg++;
    ...
    // Si aucun espace trouvé, créer un nouveau bloc
    ...
    // Lier le nouveau bloc au précédent
    if (prevBloc != -1) {
    ...
    // Écrire le nouveau bloc dans le fichier
...
void insertion_contigue_triee(ENREGISTREMENT enreg) {
    ...
    for (blocPosition = meta.adresse_premier_bloc; blocPosition < meta.adresse_premier_bloc + meta.taille_blocs; blocPosition++) {
        ...
        for (enregPosition = 0; enregPosition < bloc.nbrEnreg; enregPosition++) {
            if (bloc.enreg[enregPosition].Clé > enreg.Clé) {
                break;
            }
        }
        ...
        if (bloc.nbrEnreg < FB) {
            ...
            bloc.enreg[enregPosition] = enreg; // Insérer l'enregistrement
            bloc.nbrEnreg++;
...
void insertion_contigue_non_triee(ENREGISTREMENT enreg) {
	...
	printf("Entrer le nom du fichier ou se trouve l'enregistrement :\n");
	char nom[50];
	scanf("%49s", nom); 
	FILE *fichier = fopen(nom, "r+");
...
void defragmentation() {
	char name_file[50];
    printf("Entrer le nom du fichier a defragmenter : \n");
    scanf("%s", name_file);
             ...
             while (posBloc != -1) {
                ...
                for (int i = 0; i < bloc.nmbE; i++) {
                    if (!bloc.ENREG[i].supprime) {
                        ...
                        if (blocDest.nmbE < FB) {
                            blocDest.ENREG[blocDest.nmbE++] = bloc.ENREG[i];
                            fseek(files[i], posBlocDest * sizeof(BLOC), SEEK_SET);
                            ...
                            i--; // Réessayer d'insérer l'enregistrement dans le bloc suivant
                        }
                    }
                }
                // Passer au bloc suivant
    ...
    do{
		...
		switch(choix){
			...
			case 2: // creation d'un fichier
			    creer_fichier();
			    break;
			case 3: // affichage graphiquement l'etat de la memoire secondaire
			...
			case 5: // recherche enregistrement
			    printf("Entrer l'id de l'enegistrement recherche :\n");
			    int id;
			    scanf("%d", &id);
			    for(int i = 0; i < file_count; i++){
	        	    if(strcmp(files[i].meta.nom_fichier, name_file) == 0){
	        	    	if(files[i].meta.organisation_globale == 1){
	        	    		if(files[i].meta.organisation_interne == 1){
	        	    			//contigu trié
	        	    			recherche_contigue_triee(int id, NULL, NULL);
							}else{
								//contigu non trié
			    ...
			    break;
			case 6: // insertion enregistrement
			    insertion_chainee_triee(ENREGISTREMENT enreg);
			    insertionChaineeNonTrie(ENREGISTREMENT enreg);
			    insertion_contigue_triee(ENREGISTREMENT enreg);
			    insertion_contigue_non_triee(ENREGISTREMENT enreg);
			    break;
			case 7: // suppression enregistrement
			    int identifiant;
			    printf("Entrer l'id de l'enregistrement a supprimer :\n");
			    scanf("%d", &identifiant);
